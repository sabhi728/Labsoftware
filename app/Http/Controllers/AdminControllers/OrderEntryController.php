...
class OrderEntryController extends CommonController
{
    public function index()
    {
        ...
        return view('admin.OrderEntry.oe_index', compact('user', 'isDuplicateOrdersAllowed'));
    }
    ...
    public function indexSearch($search)
    {
        ...
        return view('admin.OrderEntry.oe_index', compact('user', 'patientDetails', 'isDuplicateOrdersAllowed'));
    }
    ...
    public function addPatientIndex()
    {
        ...
        return view('admin.OrderEntry.oe_add_patient', compact('user', 'isPatientPhoneRequired'));
    }
    ...
    public function indexBillDetails($billNo)
    {
        ...
        return view('admin.OrderEntry.oe_bill_details', compact('user', 'orderDetails', 'isOrderOnlyConsulting', 'createdAtFormatted', 'payMode'));
    }

    public function cancelBill($billNo)
    {
        $user = HomeController::getUserData();
    ...
    public function addPatient(Request $request)
    {
        ...
        if ($patient->save()) {
            if ($request->has('search')) {
                return redirect('orderentry/index/' . $umrNumber);
            }
    ...
    public function addOrderEntry(Request $request)
    {
        ...
        if (empty($request->selectedReferredById)) {
            if (!empty($request->selectedReferredByName)) {
                $isReferralCompanyExist = ReferralCompany::where('name', $request->selectedReferredByName)->first();

                if (!$isReferralCompanyExist) {
                    $referralCompany = new ReferralCompany();
                    $referralCompany->name = $request->selectedReferredByName;
                    $referralCompany->save();

                    $selectedReferredById = $referralCompany->id;
                } else {
                    $selectedReferredById = $isReferralCompanyExist->id;
                }
            }
        } else {
            $selectedReferredById = $request->selectedReferredById;
        }
        ...
        $orderEntry->referred_by = $request->referredByInput;
        ...
        if ($orderEntry->save()) {
            ...
            return redirect('orderentry/bill_details/' . $newBillNo);
        }

        return redirect()->back()->withInput();
    }

    public function updateOrderEntry($billNo, Request $request)
    {
        $user = HomeController::getUserData();
        ...
        $orderEntry->order_ids = $request->selectedOrderCommaSeparatedValues;
        $orderEntry->order_amount = $request->selectedOrderAmountsCommaSeparatedValues;
        $orderEntry->doctor = $request->selectedDoctorId;
        $orderEntry->referred_by = $request->referredByInput;
        $orderEntry->referred_by_id = $request->selectedReferredById;
        $orderEntry->total_bill = $request->totalBill;
        $orderEntry->paid_amount = $request->paidAmount == "" ? "0" : $request->paidAmount;
        $orderEntry->status = "process";
        ...
        if ($orderEntry->save()) {
            $orderEntryTransaction = OrderEntryTransactions::where('bill_no', $billNo)->first();
            $orderEntryTransaction->amount = $request->paidAmount == "" ? "0" : $request->paidAmount;
            $orderEntryTransaction->payment_method = $request->paymentMethodSelect;
            $orderEntryTransaction->txn_id = $request->paymentNumber;
            $orderEntryTransaction->save();

            if ($request->has('sendUpdateSMS') && $request->sendUpdateSMS == "true") {
                $this->sendUpdateSMS($billNo);
            }

            return redirect('orderentry/bill_details/' . $billNo);
        }
